...
const COVID_JSON_PATH = "covidData.json";
    ...
    (err, res) => {
      ...
      if (rows.length) {
        let headers = rows[0]
          .concat(rows[1])
          .filter(
            i => i.length !== 0 && i !== "MOHFW Data" && i !== "State/Region"
          );
        ...
        covidnumbers.map((item, index) => {
          covidStates = {
            [states[index]]: {
              ...Object.fromEntries(zip(headers, item, states[index]))
            },
            ...covidStates
          };
        });
...
const getOthers = token => {
  ...
  sheets.spreadsheets.values.get(
    ...
    (err, res) => {
      ...
      fs.writeFile(COVID_JSON_PATH, JSON.stringify(covid19), err => {
        if (err) return console.error(err);
        console.log("Response object stored to", COVID_JSON_PATH);
      });
    }
  );
};
